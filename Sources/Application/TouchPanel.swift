...
enum TouchPanel {
    nonisolated(unsafe) static var touchPanel: I2C!
    nonisolated(unsafe) static var buf: [UInt8]!

    static func initialize() {
        touchPanel = I2C.init(interface: .i2c3, deviceAddress: 0x38)
        buf = [UInt8](repeating: 0, count: 7)
    ...
    static func readTouchData() -> TouchData {
        ...
        let touches = TouchData(numberOfTouchPoints: Int(buf[2] & 0b1111), x: y, y: x)
        for i in 0..<7 {
            buf[i] = 0
        }
        return touches
